...
function openEditCertificate(certificateId) {
    ...
    document.getElementById('edit_indigency_purpose').value = row.cells[9].textContent.trim();
    document.getElementById('edit_indigency_other_details').value = row.cells[10].textContent.trim();
    document.getElementById('edit_id_type').value = row.cells[11].textContent.trim();

    const imageCell = row.cells[12].querySelector('img');
    if (imageCell) {
        const imagePath = 'crudIndigency/uploads/';
...
async function updateCertificate() {
    ...
    try {
        const response = await fetch('crudIndigency/update_cert.php', {
            method: 'POST',
            body: formData,
        });
