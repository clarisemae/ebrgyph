// Function to sort the Certificate of Comelec Registration table based on dropdown selection
    ...
    rows.sort((a, b) => {
        ...
        switch (criteria) {
            ...
            case 'name':
                ...
                break;
            case 'date':
                aValue = new Date(a.cells[9].textContent.trim()); // Requested Date column
                bValue = new Date(b.cells[9].textContent.trim());
                break;
            case 'remarks':
                aValue = a.cells[6].textContent.trim(); // Remarks column
    ...
    document.getElementById('search-certificates').addEventListener('input', function () {
        ...
        if (!isAnyRowVisible) {
            if (!document.getElementById('no-records')) {
                ...
                noRecordsRow.innerHTML = `<td colspan="15" style="text-align: center;">No records found</td>`;
