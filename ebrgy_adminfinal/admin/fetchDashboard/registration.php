...
try {
    $stmt = $conn->prepare("SELECT COUNT(*) AS count FROM certificate_of_comelec_registration WHERE barangay_id = ?");
    $stmt->bind_param("i", $barangayId);
    $stmt->execute();
    $stmt->bind_result($count);
    $stmt->fetch();

    echo json_encode(['certificate_of_comelec_registration' => $count]);
} catch (Exception $e) {
