...
try {
    ...
    // Query to fetch incident report count for the selected barangay
    $stmt = $conn->prepare("SELECT COUNT(*) AS count FROM incident_report WHERE barangay_id = ?");
    $stmt->bind_param("i", $barangayId);
    $stmt->execute();
    $stmt->bind_result($count);
    $stmt->fetch();
    $response['incident_report'] = $count;
