// Open Edit Modal and Populate Fields
function openEditCertificate(certificateId) {
    const editModal = new bootstrap.Modal(document.getElementById('editRecordModal'));
    editModal.show();

    const row = document.querySelector(`tr[data-id="${certificateId}"]`);
    if (!row) {
        console.error(`Row not found for certificate ID: ${certificateId}`);
        return;
    }

    // Populate modal fields
    document.getElementById('edit_id').value = certificateId;
    document.getElementById('edit_document_type').value = row.cells[1].textContent.trim();
    document.getElementById('edit_fullname').value = row.cells[2].textContent.trim();
    document.getElementById('edit_age').value = row.cells[3].textContent.trim();
    document.getElementById('edit_status').value = row.cells[4].textContent.trim();
    document.getElementById('edit_citizen').value = row.cells[5].textContent.trim();
    document.getElementById('edit_postal_address').value = row.cells[6].textContent.trim();
    document.getElementById('edit_requested_date').value = row.cells[7].textContent.trim();
    document.getElementById('edit_email').value = row.cells[8].textContent.trim();
    document.getElementById('edit_national_id_purpose').value = row.cells[9].textContent.trim();
    document.getElementById('edit_other_purpose').value = row.cells[10].textContent.trim();
    document.getElementById('edit_id_type').value = row.cells[11].textContent.trim();
    document.getElementById('edit_subject_fullname').value = row.cells[12].textContent.trim();
    document.getElementById('edit_subject_dob').value = row.cells[13].textContent.trim();
    document.getElementById('edit_subject_age').value = row.cells[14].textContent.trim();
    document.getElementById('current_id_photo').src = 'crudNationalID/uploads/' + row.cells[15].querySelector('img').src.split('/').pop(); 
}

// Update Certificate
async function updateCertificate() {
    const form = document.getElementById('editRecordForm');
    const formData = new FormData(form);

    // Debugging: Log form data to the console
    for (let [key, value] of formData.entries()) {
        console.log(`${key}: ${value}`);
    }

    try {
        const response = await fetch('crudNationalID/update_certs.php', {
            method: 'POST',
            body: formData,
        });

        if (!response.ok) {
            throw new Error(`HTTP error! status: ${response.status}`);
        }

        const result = await response.json();
        console.log('Response Data:', result);

        if (result.status === 'success') {
            alert(result.message);
            location.reload(); // Reload the page to reflect updates
        } else {
            alert(result.message || 'Failed to update the certificate.');
        }
    } catch (error) {
        console.error('Error updating certificate:', error);
        alert('An unexpected error occurred. Check the console for more details.');
    }
}