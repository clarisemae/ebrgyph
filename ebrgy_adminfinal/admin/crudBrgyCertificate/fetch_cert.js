...
async function fetchCertificates() {
  try {
    const response = await fetch("crudBrgyCertificate/fetch_cert.php"); // Adjust path as needed
    if (!response.ok) {
...
function renderCertificateTable(certificates) {
  ...
  // Clear existing rows before adding new ones (but don't overwrite with innerHTML)
  tableBody.innerHTML = "";
  ...
  certificates.forEach((cert) => {
    const row = `
        <tr data-id="${cert.id}">
          ...
          <td>${cert.email}</td>
          <td>${cert.barangay_certificate_purpose || "N/A"}</td>
          <td>${cert.barangay_other_details || "N/A"}</td>
          <td>${cert.id_type}</td>
          <td>
            ${
              cert.id_photo_url
          ...
          <td>
            <button class="btn btn-info btn-sm" onclick="openEditCertificate(${cert.id})">Edit</button>
            <button class="btn btn-danger btn-sm" onclick="deleteCertificate(${cert.id})">Delete</button>
            <button class="btn btn-warning btn-sm" onclick="printCertificate(${cert.id})">Print</button>
            <input type="checkbox" 
    ...
    // Add the row using insertAdjacentHTML (no overwriting of existing content)
          ...
          <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content">
              ...
              <div class="modal-body">
                <img src="crudBrgyCertificate/${cert.id_photo_url}" alt="Full ID Photo" class="img-fluid">
              </div>
            </div>
          </div>
        </div>
...
async function markAsDone(id, isChecked) {
  try {
    const response = await fetch("./sendBrgyCert.php", {
      method: "POST",
      headers: {
