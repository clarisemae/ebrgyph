...
function openEditCertificate(certificateId) {
    ...
    document.getElementById('edit_document_type').value = row.cells[1].textContent.trim();
    ...
    document.getElementById('edit_postal_address').value = row.cells[6].textContent.trim();
    ...
    document.getElementById('edit_non_residency_purpose').value = row.cells[9].textContent.trim();
    document.getElementById('edit_other_purpose').value = row.cells[10].textContent.trim();
    document.getElementById('edit_id_type').value = row.cells[11].textContent.trim();
    document.getElementById('edit_subject_fullname').value = row.cells[12].textContent.trim();
    document.getElementById('edit_subject_dob').value = row.cells[13].textContent.trim();
    document.getElementById('edit_subject_age').value = row.cells[14].textContent.trim();
    document.getElementById('current_id_photo').src = 'crudNonResidency/uploads/' + row.cells[15].querySelector('img').src.split('/').pop(); 
}
...
async function updateCertificate() {
    ...
    try {
        const response = await fetch('crudNonResidency/update_cert.php', {
            method: 'POST',
            body: formData,
        });
