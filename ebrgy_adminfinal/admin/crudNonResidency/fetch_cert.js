// Fetch Certificate of Non-Residency Records
async function fetchCertificates() {
  try {
    const response = await fetch("crudNonResidency/fetch_cert.php"); // Adjusted path
    if (!response.ok) {
  ...
  certificates.forEach((cert) => {
    const row = `
      <tr data-id="${cert.id}">
          ...
          <td>${cert.email}</td>
          <td>${cert.non_residency_purpose || "N/A"}</td>
          ...
          <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content">
              ...
              <div class="modal-body">
                <img src="crudNonResidency/${cert.id_photo_url}" alt="Full ID Photo" class="img-fluid">
              </div>
            </div>
          </div>
        </div>
