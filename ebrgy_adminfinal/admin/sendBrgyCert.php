...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($data['id'], $data['is_checked'])) {
        ...
        $query = "UPDATE barangay_certificate SET is_checked = ? WHERE id = ?";
        ...
        if ($stmt->execute()) {
            if ($is_checked === 1) {
                ...
                $select_query = "SELECT fullname, email FROM barangay_certificate WHERE id = ?";
...
function sendEmail($options)
{
    ...
    try {
        ...
        $mail->setFrom('BarangayOfficials@gmail.com', 'Officials');
